...
def mostrar_los_entrenadores_q_tienen_pokemones_repetidos(lista):
    i=0
    i2=0
    while i<lista.tamanio():    #barrido q recorre todos los entrenadores
        dato=lista.obtener_elemento(i)      #obtengo el dato
        ...
        while i2<ubi.sublista.tamanio():     #barrido q recorre los pokemones(en la sublista)
            dato2=ubi.sublista.obtener_elemento(i2)
            vector.append(dato2.nombre)                  #paso los nombre a un vector
            i2+=1

        #coparo en el vector si hay nombres repetidos
        i2=0
        while i2<len(vector):    #toma el un elemento del vector
            i3=0
            datov=vector[i2]
            comparador=datov 
            repetido=-1
            while i3<len(vector):   #a ese elemento lo compara con el resto de los elemnetos del vector
                if comparador==vector[i3]:  
                    repetido+=1             
                i3+=1
            i2+=1
            if repetido>0:  #si ya se repite al menos uno ya esta
                break
        
        if repetido>=1:
            print('el entrenador:',dato.nombre,' tiene al menos un pokemon q se repite')
        else:
            print('el entrenador:',dato.nombre,' no tiene pokemones q se repitan')
        vector=[]        
        i+=1
        i2=0
        i3=0
...
print('I')
mostrar_los_entrenadores_q_tienen_pokemones_repetidos(lista_de_entrenadores)
print('J')
